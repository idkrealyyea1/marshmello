...
(function initNav() {
  ...
  window.addEventListener('scroll', () => {
    // Don't auto-hide/transform the navbar while the mobile menu is open —
    // .nav-links is fixed-positioned relative to #navbar's transform context,
    // so moving the navbar would drag the open menu off-screen with it.
  ...
  if (toggle && links) {
    toggle.addEventListener('click', () => {
      ...
      // Reset any hide-transform so the menu isn't offset when it opens
      navbar.style.transform = 'translateY(0)';
    });
